...
benchmark_secp256k1_report: ## Compare secp256k1 implementations with formatted report
	...
	@echo "=================================================================="
	@timeout 60s go test -bench="BenchmarkKeyGeneration|BenchmarkSigning|BenchmarkVerification" -benchmem -run=^$$ -benchtime=3s 2>/dev/null | python3 format_benchmark.py || (echo "⚠️  Benchmark timed out or failed. Trying without Ethernet library..." && CGO_ENABLED=0 go test -bench="BenchmarkKeyGenerationNoCgo|BenchmarkSigningNoCgo|BenchmarkVerificationNoCgo" -benchmem -run=^$$ -benchtime=3s 2>/dev/null | python3 format_benchmark.py)
	@echo "=================================================================="
	@echo "💡 Key Insights:"
